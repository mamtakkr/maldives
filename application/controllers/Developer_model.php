...
class Developer_model extends CI_Model {  
    ...
    public function get_blogs($offSet = 0, $perPage = 0) {
        $this->db->select("news_blog.*, CONCAT(".DB_PRE."users.first_name, ' ', ".DB_PRE."users.last_name) AS user_name");
    ...
    public function home_resorts($offSet = 0, $perPage = 0, $feachered='') { 
        ...
        $this->db->select("resorts.*,(SELECT state_name from mal_states where mal_states.id=mal_resorts.physical_state) as atoll,(SELECT GROUP_CONCAT(".DB_PRE."holidays.holiday_name SEPARATOR ', ') FROM ".DB_PRE."holidays 
    ...
    public function resort_accommodation($resort_id='', $limit=0, $counter='') {
		...
		if($this->input->post('room_count')){
			$room_array = explode(",",implode(',',$this->input->post('room_count')));
    ...
    public function resort_dinnings($resort_id='', $limit=0, $counter='') {
        ...
        if(!empty($mealStatus)&&($mealStatus=='no' || !empty($meal_served_where))){
            ...
            if($this->input->get('food_type')){
                ...
                $vegetarianResortIDs = $this->where("(".implode(' OR ', $food_type_where).")");
            }
    ...
    public function get_traveller_story_row() {
	 ...
	 public function inspiration_accommodation($offSet, $per_page) {
		...
		$acc_cat_id =  $this->input->post('acc_cat_id')?$this->input->post('acc_cat_id'):'';
        $villa_types_where = $locations_where = $facilitys_where = array();      
        $this->db->select("accommodation.*, (SELECT GROUP_CONCAT(".DB_PRE."ideals.ideal_name SEPARATOR ', ') FROM ".DB_PRE."ideals 
		WHERE FIND_IN_SET(".DB_PRE."ideals.id, ".DB_PRE."accommodation.ideal_for)) as ac_ideals, 
		(SELECT GROUP_CONCAT(".DB_PRE."amenities.amenitie_name SEPARATOR ', ') FROM ".DB_PRE."amenities WHERE FIND_IN_SET(".DB_PRE."amenities.id, ".DB_PRE."accommodation.amenities)) as ac_amenities, villa_type.villa_type, resort_category, resort_name,name_of_villa, state_name, resort_description,island_name,villa_type.villa_type as villa_type_name"); 
		...
		if($acc_cat_id!=''){
			$this->db->where('accommodation.villa_type', $acc_cat_id);
		}
		...
		if($this->input->get('acc_sports')){
            $acc_sports = $this->input->get('acc_sports');
            if(!empty($acc_sports)){
		...
		if($this->input->post('acc_room_count')){
			$room_array = explode(",",implode(',',$this->input->post('acc_room_count')));
			$min_room_count = min($room_array);
			$max_room_count = max($room_array);
			if(isset($min_room_count) && $max_room_count<151){
			 $room_count_where[] = " accommodation.number_of_rooms_per_villa < ".$min_room_count;
	...
	// Exprience
	 public function inspiration_experience() {
		...
		if($this->input->post('exp_room_count')){
			$room_array = explode(",",implode(',',$this->input->post('exp_room_count')));
			$min_room_count = min($room_array);
			$max_room_count = max($room_array);
			if(isset($min_room_count) && $max_room_count<151){
	...
	public function get_compare_villa($id_array = '', $in_data='') { 
        $this->db->select('acc.id,GROUP_CONCAT(acc.room_size," ", "sqm,",acc.number_of_rooms_per_villa," ","Bedroom"," ","Villa,", villatype.villa_type) as villa_type_name');
		...
		$min_room_count = $this->input->post('room_min_val')?$this->input->post('room_min_val'):'';
		...
		if($min_room_count !='' && $max_room_count!=''){
			...
			if(isset($max_room_count))
			 $room_count_where[] = " acc.number_of_rooms_per_villa < ".$max_room_count;
			 $this->db->where("(".implode(' AND ', $room_count_where).")");
		}
		
        $this->db->group_by('acc.id');
        $this->db->order_by('villatype.villa_type', 'ASC');
        $query = $this->db->get();
	//	echo $this->db->last_query();//die;
        if ($query->num_rows() > 0) return $query->result();
        else return FALSE;
