...
<section class="content-header" id="filter_main">
  <form action="<?php echo ADMIN_URL.'Amenities/categorys';?>" method="get">
    <div class="filter_box" style="width:180px;">
      <label>Category ID</label>
      <input type="text" name="categorty_id" value="<?php if($this->input->get('categorty_id')){echo $this->input->get('categorty_id');}?>" class="form-control"  placeholder="Search Category ID">
    </div>
            ...
            if(!empty($rows)){
              foreach ($rows as $res_data){?>
                <tr>
                    ...
                    <div class="dropdown">
                      ...
                      <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                        ...
                        <li role="presentation">
                          <a role="menuitem" tabindex="-1" href="javascript:void(0);" onclick="changeStatus('amenities_category','Category','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','<?php echo $k; ?>','<?php if($k==1){ echo 'activate';}else if($k==3){ echo 'delete';}else{ echo 'deactive';}?>','id','status');">
                            <?php echo $status_a; ?>
                          </a>
                        </li>
                        <?php } 
                  ...
                  <td class="text-center"> 
                    <a class="btn btn-sm btn-primary" href="<?php echo ADMIN_URL.'amenities/add_category/';if(!empty($res_data->id)) echo $res_data->id;?>">
                        <i class="fa fa-pencil" aria-hidden="true"></i>
                     </a>   
                     <a class="btn btn-sm btn-danger" href="javascript:void(0);" onclick="changeStatus('amenities_category','Category','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','3','delete','id','status');");">
