...
<section class="content-header" id="filter_main">
  <form action="<?php echo ADMIN_URL.'brands';?>" method="get">
    <div class="filter_box" style="width:150px">
      <label>Brand ID</label>
      <input type="text" name="brand_id" value="<?php if($this->input->get('brand_id')){echo $this->input->get('brand_id');}?>" class="form-control"  placeholder="Search Brand ID">
    </div>
    <div class="filter_box" style="width:">
      <label>Brand Name</label>
      <input type="text" name="brand_name" value="<?php if($this->input->get('brand_name')){echo $this->input->get('brand_name');}?>" class="form-control"  placeholder="Search Brand">
    </div>
    <div class="filter_box" style="width:">
      <label>Order</label>
              ...
              foreach ($rows as $res_data){?>
                <tr>
                  ...
                  <td class="text-center">
                   ...
                   <td class="text-left">
                    <?php  echo !empty($res_data->brand_name)? ucfirst($res_data->brand_name):'-';?>
                  </td>                              
                  <td class="text-center"> 
                    <div class="dropdown">
                      ...
                      <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                        ...
                        <li role="presentation">
                          <a role="menuitem" tabindex="-1" href="javascript:void(0);" onclick="changeStatus('brand','Brand','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','<?php echo $k; ?>','<?php if($k==1){ echo 'activate';}else if($k==3){ echo 'delete';}else{ echo 'deactive';}?>','id','status');">
                            <?php echo $status_a; ?>
                          </a>
                        </li>
                        <?php } 
                  ...
                  <td class="text-center"> 
                    <a class="btn btn-sm btn-primary" href="<?php echo ADMIN_URL.'brands/addBrand/';?><?php if(!empty($res_data->id)) echo $res_data->id;?>">
                        <i class="fa fa-pencil" aria-hidden="true"></i>
                     </a>   
                     <a class="btn btn-sm btn-danger" href="javascript:void(0);" onclick="changeStatus('brand','Brand','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','3','delete','id','status');");">
