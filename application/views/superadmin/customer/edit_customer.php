...
#governmentFile_pic img {
  width: 100px !important;
  padding: 0 !important;
}
#governmentFile_pic img, #governmentFile_pic iframe{
  width: 100%;
  padding: 20px 15%;
  border: none;
}
        ...
        <!-- form start -->
          <?php 
            ...
            if(!empty($user->government_id_number)){
              echo '<input type="hidden" name="old_government_id_number" value="'.$user->government_id_number.'">';
            }  
            if(!empty($user->id)){
          ...
          <div class="box-body">
            ...
            <div class="form-group">
              <label for="exampleInputEmail1">
                Email Address1 <span class="text-danger">*</span>
              </label>                  
            ...
            <div class="form-group">
              ...
              <div id="governmentFile_pic">
                <?php 
                if(!empty($user->id)){
                  $images = $this->common_model->get_result('images', array('user_id'=>$user->id));
                  if(!empty($images)){
                    foreach($images as $image){
                      $deletImg = "deleteParmanetImg('".$image->id."', '".$image->image_name."');";
                      echo '<div class="uplod-wrapp" id="'.$image->id.'">
                              <a href="javascript:void(0);" onclick="'.$deletImg.'"  class="photo-remove">
                      ...
                      if(!empty($fileNames)&&($fileNames=='png'||$fileNames=='jpeg'||$fileNames=='jpg')){
                        echo '<img src="'.base_url().'uploads/users/thumbnails/'.$image->image_name.'"/>';
                      }else if(!empty($fileNames)&&($fileNames=='pdf')){
                        echo '<iframe src="'.base_url().'uploads/users/'.$image->image_name.'"/>';
                      }else if(!empty($fileNames)&&($fileNames=='doc'||$fileNames=='docx')){
                        echo '<a href="'.base_url().'uploads/users/'.$image->image_name.'"><img src="'.base_url('assets/front/images/word-download.png').'"/></a>';
                      }
                      echo '</div>';
                    }                                
                  }
...
<script type="text/javascript">
  ...
  function deleteParmanetImg(imageID, fileImg){
    alertify.confirm("Are you sure you want to delete this image?", function (e) {
      if (e) {
        $.ajax({
          url: "<?php echo base_url();?>home/deleteLocalImg",
          type:'POST',
          data:{"fileImg":fileImg,"imageID":imageID}, 
