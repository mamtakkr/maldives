...
if(!empty($comments)){
	foreach($comments as $comment){?>
        ...
        <a href="javascript:void(0);" onclick="blog_comment_status('<?php echo !empty($comment->id)?$comment->id:''; ?>');" class="story_comment_status" id="story_comment_status_<?php echo !empty($comment->id)?$comment->id:''; ?>" data="<?php echo (!empty($comment->status)&&$comment->status==1)?"2":"1";?>"><?php echo (!empty($comment->status)&&$comment->status==1)?"block":"active";?></a>
      </div>
	<?php 
	}
}?>