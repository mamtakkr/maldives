...
<section class="content-header" id="filter_main">
  <form action="<?php echo ADMIN_URL."resorts/traveller_story" ?>" method="get">
              ...
              foreach ($rows as $res_data){?>
                <tr>
                 ...
                 <td class="text-center"> 
                    <div class="dropdown">
                      <button class="<?php echo btnOrder($res_data->verified_status);?> btn-xs  dropdown-toggle_get_val" id="menu1" type="button" data-toggle="dropdown">
                      <?php
                          ...
                          if(!empty($res_data->verified_status) && $k != $res_data->verified_status && $k != 4){
                            if($k==1){?>
                              <li role="presentation">
                                <a role="menuitem" tabindex="-1" href="javascript:void(0);" onclick="ApproveReviewStatus1('<?php if(!empty($res_data->id)) echo $res_data->id; ?>');">
                                  <?php echo $status_a; ?>
                                </a>
                              </li>
                        <?php 
                              ...
                              <li role="presentation">
                                <a role="menuitem" tabindex="-1" href="javascript:void(0);" onclick="changeStatus('traveller_stories','story','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','<?php echo $k; ?>','<?php if($k==3){ echo 'delete';}else{ echo 'reject';}?>','id','verified_status');">
                                  <?php echo $status_a; ?>
                                </a>
                              </li>
                            <?php 
                  ...
                  <td class="text-center"> 
                      ...
                      <a class="btn btn-sm btn-danger" href="javascript:void(0);" onclick="changeStatus('traveller_stories','story','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','3','delete','id','status');")>
...
<div class="modal fade" id="review_status_modal" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="myModalLabel">Approve  Traveller Story</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="review_status" method="post" action="<?php echo ADMIN_URL.'resorts/approve_traveller_story' ?>">
          <select name="verified_by" class="form-control" required="required">
...
<script type="text/javascript">
  function  review_details_modal(story_id) {
     $.ajax({ 
      url:"<?php echo ADMIN_URL;?>resorts/review_details_modal",
      type:"POST",
